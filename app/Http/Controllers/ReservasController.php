...
class ReservasController extends Controller
{
    ...
    private function nuevos_grupos_manual($data, $id_reserva, $cliente, $data_detail)
    {
        ...
        $Porcentaje_monto_total = 10;
        $cantidad_x_persona_adulto = 10;
        $cantidad_x_persona_nino = 10;
        $cantidad_fija_x_habitacion = 10;
    ...
    private function CrearReservaDetallesManual($id_grupos, $ROOM_TYPE, $data_detail, $id_reserva, $cliente, $Porcentaje_monto_total, $cantidad_x_persona_adulto, $cantidad_x_persona_nino, $cantidad_fija_x_habitacion, $cuenta_id)
    {
        ...
        for ($i = $data_detail['ingreso']; $i < $data_detail['salida']; $i = date("Y-m-d", strtotime($i . "+ 1 days"))) {
            ...
            $id_reservas = TblReservasGrupo::where('id_reservas', '=', $reserva->id)
                ->first();
            $result_adulto = ((int)$data_detail['cantidad_adultos'] * (float)1);
            $result_nino = ((int)$data_detail['cantidad_ninos'] * (float)1);
            ...
            $ReservasDetalle->tarifa_neta = ((float)$tarifa) / (1 + (1 / 100));
            $ReservasDetalle->tasa_impuestos = ($ReservasDetalle->check_in_fecha == $data_detail['ingreso']) ? (((float)$ReservasDetalle->tarifa_neta * (float)$decimal) + (float)1) + ($result_adulto) + ($result_nino) : ((float)$ReservasDetalle->tarifa_neta * (float)$decimal) + $result_adulto + $result_nino;
    ...
    private function newGrupoManual($id_reserva, $data, $Porcentaje_monto_total, $cantidad_x_persona_adulto, $cantidad_x_persona_nino, $cantidad_fija_x_habitacion, $data_detail)
    {
        ...
        $adulto = ((int)$data_detail['cantidad_adultos'] * ($dias->days));
        $nino = ((int)$data_detail['cantidad_ninos'] * (float)1) * ($dias->days);
        $valor_neto = ($data_detail['total_reserva']) / (1 + (1 / 100));
    ...
    private function DetalleReserva($data)
    {
        $total = TblReservasDetalle::where('tbl_reservas_grupo.id_reservas', '=', $data->id)
            ...
            ->count();
        $res = TblReservasDetalle::select(
            'tbl_reservas_detalle.id as id_reservas_detalle',
            'tbl_reservas_detalle.id_habitacion',
            'tbl_reservas_detalle.id as reserva_detalle',

            'tbl_habitaciones_tipo.id as id_habitacion_tipo',
            'tbl_habitaciones_tipo.codigo',
            'tbl_habitaciones_tipo.nombre as habitacion_nombre',

            DB::raw('tbl_reservas_detalle.precio_total'),
            DB::raw('tbl_reservas_detalle.precio_neto'),
            DB::raw('coalesce(tbl_reservas_detalle.infantes_cantidad,0) AS infantes_cantidad'),
            DB::raw('coalesce(tbl_reservas_detalle.ninos_cantidad,0) AS ninos_cantidad'),
            DB::raw('coalesce(tbl_reservas_detalle.adultos_cantidad,0) AS adultos_cantidad'),
            DB::raw('coalesce(tbl_reservas_detalle.huespedes_cantidad,0) AS huespedes_cantidad')
        )->where('tbl_reservas_grupo.id_reservas', '=', $data->id)
            ->whereNull('tbl_reservas_detalle.id_habitacion')
            ->join('tbl_habitaciones_tipo', 'tbl_reservas_detalle.id_habitacion_tipo', '=', 'tbl_habitaciones_tipo.id')
            ->join('tbl_reservas_grupo', 'tbl_reservas_detalle.id_reservas_grupo', '=', 'tbl_reservas_grupo.id')
            ->first();
    ...
    private function CrearReservaDetalles($id_grupos, $ROOM_TYPE, $id_reserva, $cliente, $Porcentaje_monto_total, $cantidad_x_persona_adulto, $cantidad_x_persona_nino, $cantidad_fija_x_habitacion, $cuenta_id, $collection, $accion)
    {
        ...
        $reserva = TblReserva::find($id_reserva);
        $precio_total = (float)$ROOM_TYPE->Average_Rate;
        ...
        foreach ($idReservasDetalles as $ids) {
            foreach ($request->huespedes_data as $temp) {
                ...
                if (!is_null($temp->id_huesped) && !is_null($ids->id)) {
                    ...
                    $new->Save();
                    unset($new);
                    $pagador = TblReservaPagador::firstOrNew([
                        'cliente_id' => $temp->id_huesped,
    ...
    private function datos($id)
    {
        ...
        $pagadores = TblReservaPagador::select(
            'tbl_reservas_pagadores.id',
            'tbl_reservas_pagadores.cliente_id',
            'tbl_reservas_pagadores.reserva_detalle_id',
            'tbl_reservas_pagadores.valor_a_pagar',
            'tbl_clientes.id as identificador_cliente',
            'tbl_clientes.nombre',
            'tbl_clientes.apellido',
            'tbl_reservas_pagadores.reserva_grupo_id'
    ...
    public function reservas_grupos(Request $request)
    {
        if ($request->habitacion == 'todas') {
            ...
            $reservas = TblReservasDetalle::select(
                'tbl_reservas.id as reserva_id',
                ...
                'tbl_reservas_detalle.check_out_fecha',
                'tbl_reservas_detalle.numero_impuesto',
                'tbl_reservas_detalle.numero_impuesto2',
                'tbl_reservas_detalle.servicio',
                'tbl_habitaciones.numero'
            )
    ...
    public function habitaciones_grupos(Request $request)
    {
        ...
        $habitaciones = TblReservasDetalle::select(
            'tbl_habitaciones.id',
            'tbl_habitaciones.numero'
        )
    ...
    public function cancelar_factura(Request $request)
    {
        ...
        foreach ($request->datos as $value) {
            ...
            $EstadoFactura = TblReservasDetalle::select(
                'id_reservas_grupo',
                'id_habitacion_tipo',
                'adultos_cantidad',
                'ninos_cantidad',
    ...
    private function datos_pagadores_facturas($id)
    {
        ...
        $pagadores = PagadorFactura::select(
            ...
            'tbl_pagadores_facturas.cliente_id',
            'tbl_clientes.nombre',
            'tbl_clientes.apellido',
            'tbl_reservas_detalle.id_reservas_grupo',
            DB::raw("SUM(tbl_pagadores_facturas.valor_a_pagar) as valor_a_pagar"),
            'tbl_pagadores_facturas.id as id_tbl_pagador_factura',
            'tbl_pagadores_facturas.valor_pagado',
            'tbl_reservas_detalle.check_in_fecha',
            'tbl_reservas_detalle.id as reservas_detalle_id',
            'tbl_facturas_detalles.numero_impuesto1',
            'tbl_facturas_detalles.numero_impuesto2',
    ...
    public function reserva_factura(Request $request)
    {
        ...
        $FacturaDetalle = FacturaDetalle::select(
            DB::raw("SUM(tbl_facturas_detalles.precio_bruto) as precio_bruto"),

            'tbl_habitaciones.numero',
            'tbl_reservas_grupo.id',
            'tbl_facturas_detalles.factura_id',
            'tbl_facturas_detalles.numero_impuesto1',
            'tbl_facturas_detalles.numero_impuesto2',
            'tbl_facturas_detalles.servicio',
            'tbl_reservas.id as clave_reserva'
    ...
    public function Resumenes($factura_detalle_id, $id_numero, $cliente)
    {
        ...
        $FacturaDetalle = FacturaDetalle::select(
            ...
            ->where('tbl_facturas_detalles.factura_id', '=', (int)$factura_detalle_id)->where('tbl_facturas_detalles.id_habitacion', '=', (int)$id_numero)->groupBy(
                'tbl_habitaciones.numero',
                'tbl_reservas_grupo.id',
                'tbl_facturas_detalles.precio_bruto',
                'tbl_facturas_detalles.factura_id'
            )
    ...
    public function pdf($factura, $reserva)
    {
        ...
        $pagadores = PagadorFactura::select(
            ...
            'tbl_reservas_detalle.check_in_fecha',
            'tbl_reservas_detalle.precio_total',
            'tbl_reservas_detalle.precio_neto as precio_neto_2',
            'tbl_reservas_detalle.id_habitacion',
            'tbl_reservas_detalle.id',
            'tbl_habitaciones.numero',
            'tbl_facturas_detalles.numero_impuesto1',
            'tbl_facturas_detalles.numero_impuesto2',
            'tbl_facturas_detalles.servicio',
            DB::raw("SUM(tbl_pagadores_facturas.valor_a_pagar) as precio_neto")
        )
            ->join('tbl_clientes', 'tbl_clientes.id', '=', 'tbl_pagadores_facturas.cliente_id')
            ->join('tbl_facturas_detalles', 'tbl_pagadores_facturas.factura_detalle_id', '=', 'tbl_facturas_detalles.id')
            ->join('tbl_reservas_detalle', 'tbl_facturas_detalles.reserva_detalle_id', '=', 'tbl_reservas_detalle.id')
    ...
    public function estadoHabitacionSalida($id_habitacion, $estadoNull, $estadoNullid, $fecha)
    {
        ...
        return (object)[
            'nombre' => $nombre, 'id' => $id, 'libre' => $libre, 'cliente' => $cliente, 'cliente_id' => $cliente_id, 'check_out_fecha' => $fecha, 'color' => $color, 'descripcion' => $descripcion, 'salida' => $salida
    ...
    public function reservanteHabitacion($id_reservas)
    {
        ...
        return (object)[
            'full_name' => $reservante->full_name
    ...
    public function VerEstadosReservas(Request $request)
    {
        ...
        $alojados = TblReservasDetalle::select(
            ...
            'tbl_reservas_detalle.id_reservas_grupo',
            'tbl_reservas_grupo.id_reservas_estado',
            'tbl_reservas_detalle.id as identificador_detalle',
            'tbl_reservas_grupo.id_reservas'
        )->join('tbl_habitaciones', 'tbl_reservas_detalle.id_habitacion', '=', 'tbl_habitaciones.id')
            ->join('tbl_habitaciones_tipo', 'tbl_habitaciones.id_habitacion_tipo', '=', 'tbl_habitaciones_tipo.id')
            ->join('tbl_reservas_grupo', 'tbl_reservas_detalle.id_reservas_grupo', '=', 'tbl_reservas_grupo.id')
...

    {
        ...
        $res = TblReservasDetalle::where('tbl_reservas_detalle.id_habitacion', '=', (int)$id_habitacion)->where('tbl_reservas_detalle.check_in_fecha', '=', $fecha)->select(
            'tbl_reservas_detalle.salida',
            'tbl_reservas_detalle.id_habitacion',
            'tbl_reservas.check_in_fecha as check_in_fecha',
            'tbl_reservas.check_out_fecha as check_out_fecha',
            DB::raw('date(now())-date(tbl_reservas_detalle.check_in_fecha) AS dias'),
            'tbl_habitaciones_estado.id',
            'tbl_habitaciones_estado.color',
            'tbl_habitaciones_estado.nombre',
            'tbl_habitaciones_detalle_estado.descripcion',

            DB::raw('concat_ws(\' \',tbl_clientes.nombre,tbl_clientes.apellido) as cliente'),
        ...
        return (object)[
            'nombre' => $nombre, 'id' => $id, 'libre' => $libre, 'libreValor' => $libreValor, 'cliente' => $cliente, 'cliente_id' => $cliente_id, 'check_in_fecha' => $check_in_fecha, 'check_out_fecha' => $check_out_fecha, 'color' => $color, 'descripcion' => $descripcion, 'salida' => $salida
    ...
    {
        $res = TblReservasDetalle::where('tbl_reservas_detalle.id_habitacion', '=', $id_habitacion)->where('tbl_reservas_detalle.check_in_fecha', '=', $fecha)->select(
            'tbl_reservas_detalle.salida',
            'tbl_reservas_detalle.id_habitacion',
            'tbl_reservas.check_in_fecha as check_in_fecha',
            'tbl_reservas.check_out_fecha as check_out_fecha',
            ...
            'tbl_habitaciones_estado.nombre',
            'tbl_habitaciones_detalle_estado.descripcion',
            'tbl_habitaciones_detalle_estado.fecha as variable',
        ...
        return (object)[
            'nombre' => $nombre, 'id' => $id, 'libre' => $libre, 'cliente' => $cliente, 'cliente_id' => $cliente_id, 'check_in_fecha' => $check_in_fecha, 'check_out_fecha' => $check_out_fecha, 'color' => $color, 'descripcion' => $descripcion, 'salida' => $salida, 'variable' => $variable
    ...
    public function clientes_hospedados(Request $request)
    {
        ...
        $clientes_hospedados = TblReservasDetalleHuespedes::select(
            'tbl_reservas_detalle_huespedes.id_cliente_huesped',
            'tbl_clientes.nombre',
            'tbl_clientes.apellido'
        )
    ...
    public function eliminar_cliente_hospedado(Request $request)
    {
        ...
        $clientes_hospedados = TblReservasDetalleHuespedes::select(
            'tbl_reservas_detalle_huespedes.id_cliente_huesped',
            'tbl_reservas_detalle_huespedes.id'
        ...
        if ($validate == true) {
            ...
            if ($data->count()) {
                ...
                $validator = Validator::make($arr, [
                    "*.fecha_ingreso" => "required|date", "*.fecha_salida" => "required|date", "*.tipo_habitacion" => "required|string", "*.cantidad_adultos" => "integer", "*.cantidad_ninos" => "integer", "*.tarifa" => "required", "*.nombres_cliente" => "required|string", "*.apellidos_cliente" => "required|string", "*.telefono" => "required|integer", "*.correo" => "required|email", "*.fuente" => "required|string",
                ]);
                if ($validator->fails()) {
    ...
    private function CrearReservaDetallesExcel($id_grupos, $ROOM_TYPE, $id_reserva, $cliente)
    {
        ...
        $reserva = TblReserva::find($id_reserva);
        $precio_total = (float)$ROOM_TYPE['tarifa'];
        $date1 = new \DateTime($reserva->check_in_fecha);
        $date2 = new \DateTime($reserva->check_out_fecha);
        $diff = $date1->diff($date2);
        $dias = $diff->days;
    ...
    public function asignar_pagos_notas(Request $request)
    {
        ...
        foreach ($datos as $dato) {
            ...
            $pago = TipoPagoTotalDetalle::firstOrNew([
                'tipo_pago_total_id' => $dato['id'], 'tipo_pago_id' => (int)$request->pagosNotas['tipo_pago_id']
